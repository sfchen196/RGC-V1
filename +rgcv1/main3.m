...
figure(2);
plot(pos_V1(:,1),pos_V1(:,2), '.b');
title('V1 constructed without RGC inputs');
...
% figure(3);
% % ax(3) = subplot(223); imagesc(opmap0); axis xy image; title('Initial'); colormap(hsv);
% % caxis([-pi/2 pi/2]); colorbar;
% ax(2) = subplot(122); imagesc(opmap); axis xy image; colormap(hsv);
...
ax_sampled = gca;
op_unwrap_filt = conv2(op_unwrap,ones(10,10)/sum(sum(ones(10,10))),'same');
figure; imagesc(-1600:1600,-1600:1600,op_unwrap_filt); caxis([-pi/2 pi/2]); colorbar; colormap(hsv); axis xy image; title('conv filter');
ax_rgc2 = gca;
linkaxes([ax_rgc2 ax_sampled]);
