...
if type == 1
    ...
    theta = 0.15*pi; rot = [cos(theta) sin(theta); -sin(theta) cos(theta)];
...
pos_OFF(pos_OFF(:,1)<-crop_x,:) = []; pos_OFF(pos_OFF(:,1)>crop_x,:) = [];
pos_OFF(pos_OFF(:,2)<-crop_y,:) = []; pos_OFF(pos_OFF(:,2)>crop_y,:) = [];
% gpuDevice(1); 
toc
end